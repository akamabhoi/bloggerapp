...
public class summary11 extends AppCompatActivity {
    private EditText text;
    private TextView summary;
    private Button summarize;
    private ElegantNumberButton mark;

    String content,final_content="",arr[];
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_summary11);
        summarize=(Button)findViewById(R.id.summarize);
        text=(EditText)findViewById(R.id.editText);
        summary=(TextView)findViewById(R.id.summary);
        mark=(ElegantNumberButton) findViewById(R.id.marks);
        summarize.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                arr=content.split(" ");
                final_content="";
                int marks=Integer.parseInt(mark.getNumber().toString());
                for(String x:arr)
                {
                    final_content+=x;
                    final_content+=" ";
                }
                String ans=Summarize(final_content,2*marks);
                summary.setText(ans);
            }
        });
    }
    ...
    public String Summarize(String text, int maxSummarySize)
    {
        ...
        for(String sentence : sentences)//foreach string sentence in sentences list
        {
            if(setSummarySentences.contains(sentence))
            {
                ...
                if(set.contains(sentence)==false) {
